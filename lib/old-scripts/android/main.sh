...
# Initialize logging function FIRST
...
error_handler() {
    ...
    log "❌ Script crashed at line $line_no"
    log "❌ Exit code: $exit_code"
    log "❌ Last command: $last_command"
    log "❌ Function stack: $func_stack"
...
if [ -f "lib/scripts/utils/force_fix_env_config.sh" ]; then
    ...
    echo "⚠️ force_fix_env_config.sh not found, skipping..."
fi
...
# CRITICAL: Validate generated environment configuration to prevent runtime crashes
log "🔍 Validating generated environment configuration for runtime stability..."
if [ -f "lib/config/env_config.dart" ]; then
    # Check for any remaining shell variable patterns that could cause crashes
    if grep -q '\$[A-Z_][A-Z0-9_]*' "lib/config/env_config.dart"; then
        log "❌ CRITICAL: Found shell variable patterns in generated Dart code!"
        log "These will cause immediate runtime crashes:"
        grep -n '\$[A-Z_][A-Z0-9_]*' "lib/config/env_config.dart"
        
        # Attempt emergency fix
        log "🔧 Attempting emergency fix..."
        ...
        log "✅ Emergency fix applied, re-validating..."
        if grep -q '\$[A-Z_][A-Z0-9_]*' "lib/config/env_config.dart"; then
            log "❌ Emergency fix failed, build will crash!"
            exit 1
        else
            log "✅ Emergency fix successful"
        fi
    else
        log "✅ No shell variable patterns found in generated code"
    fi
    
    # Validate Dart syntax to prevent compilation crashes
    if command -v flutter >/dev/null 2>&1; then
        log "🔍 Validating Dart syntax..."
        if flutter analyze "lib/config/env_config.dart" >/dev/null 2>&1; then
            log "✅ Dart syntax validation passed - APK should not crash"
        else
            log "❌ Dart syntax validation failed - APK will crash!"
    ...
    log "❌ CRITICAL: env_config.dart not generated - APK will crash!"
    exit 1
fi
...
# CRITICAL: Runtime crash prevention - validate all critical components
log "🔍 Validating critical components to prevent runtime crashes..."

# Check if main.dart exists and is valid
if [ ! -f "lib/main.dart" ]; then
    log "❌ CRITICAL: main.dart not found - APK will crash!"
    exit 1
fi

# CRITICAL: Fix push notifications before build
log "🔔 Fixing push notification permissions and configurations..."
if [ -f "lib/scripts/utils/fix_notifications.sh" ]; then
    chmod +x lib/scripts/utils/fix_notifications.sh
    if lib/scripts/utils/fix_notifications.sh; then
        log "✅ Push notification fixes applied successfully"
    else
        log "⚠️ Push notification fixes failed, but continuing with build..."
    fi
else
    log "⚠️ fix_notifications.sh not found, skipping notification fixes..."
fi

# CRITICAL: Validate ProGuard rules before build
log "🔍 Validating ProGuard rules to prevent build failures..."
if [ -f "lib/scripts/utils/validate_proguard.sh" ]; then
    chmod +x lib/scripts/utils/validate_proguard.sh
    if lib/scripts/utils/validate_proguard.sh; then
        log "✅ ProGuard rules validated successfully"
    else
        log "❌ ProGuard validation failed, attempting to fix..."
        # Try to fix ProGuard issues
        if lib/scripts/utils/validate_proguard.sh; then
            log "✅ ProGuard issues fixed successfully"
        else
            log "❌ ProGuard fixes failed, build may fail"
        fi
    fi
else
    log "⚠️ validate_proguard.sh not found, skipping ProGuard validation..."
fi

    # CRITICAL: Generate dynamic ProGuard rules before build
    log "🔧 Generating dynamic ProGuard rules to prevent build failures..."
    if [ -f "lib/scripts/utils/generate_proguard_rules.sh" ]; then
        chmod +x lib/scripts/utils/generate_proguard_rules.sh
        if lib/scripts/utils/generate_proguard_rules.sh; then
            log "✅ Dynamic ProGuard rules generated successfully"
        else
            log "❌ Dynamic ProGuard generation failed, attempting fallback..."
            # Try to use existing ProGuard rules
            if [ -f "android/app/proguard-rules.pro" ]; then
                log "⚠️ Using existing ProGuard rules as fallback"
            else
                log "❌ No ProGuard rules available, build may fail"
            fi
        fi
    else
        log "⚠️ generate_proguard_rules.sh not found, using existing ProGuard rules..."
    fi
    
    # CRITICAL: Apply comprehensive R8 missing classes fix to prevent AAB build failures
    log "🔧 Applying comprehensive R8 missing classes fix to prevent AAB build failures..."
    if [ -f "lib/scripts/utils/fix_r8_comprehensive.sh" ]; then
        chmod +x lib/scripts/utils/fix_r8_comprehensive.sh
        if lib/scripts/utils/fix_r8_comprehensive.sh; then
            log "✅ Comprehensive R8 missing classes fix applied successfully"
        else
            log "❌ Comprehensive R8 missing classes fix failed, but continuing with build..."
        fi
    else
        log "⚠️ fix_r8_comprehensive.sh not found, R8 build may fail..."
    fi
    
    # CRITICAL: Update package name in build.gradle.kts to match environment
    log "🔧 Updating package name in build configuration..."
    if [ -f "lib/scripts/utils/update_package_name.sh" ]; then
        chmod +x lib/scripts/utils/update_package_name.sh
        if lib/scripts/utils/update_package_name.sh; then
            log "✅ Package name updated successfully"
        else
            log "❌ Package name update failed, but continuing with build..."
        fi
    else
        log "⚠️ update_package_name.sh not found, package mismatch may cause issues..."
    fi

# Check if env_config.dart is properly generated
if [ ! -f "lib/config/env_config.dart" ]; then
    log "❌ CRITICAL: env_config.dart not generated - APK will crash!"
    exit 1
fi

# Validate that env_config.dart contains no shell variables
if grep -q '\$[A-Z_][A-Z0-9_]*' "lib/config/env_config.dart"; then
    log "❌ CRITICAL: Shell variables found in env_config.dart - APK will crash!"
    log "Found variables:"
    grep -n '\$[A-Z_][A-Z0-9_]*' "lib/config/env_config.dart"
    exit 1
fi

# Check if critical Flutter files exist
CRITICAL_FILES=(
    "lib/main.dart"
...
for file in "${CRITICAL_FILES[@]}"; do
    if [ ! -f "$file" ]; then
        log "❌ CRITICAL: $file not found - APK will crash!"
        exit 1
    fi
done

log "✅ All critical components validated - APK should not crash"
...
# CRITICAL: Post-build APK validation to prevent crashes
log "🔍 Validating generated APK to prevent runtime crashes..."

# Check if APK exists and has reasonable size
if [ -f "output/android/app-release.apk" ]; then
    APK_SIZE=$(stat -c%s "output/android/app-release.apk" 2>/dev/null || stat -f%z "output/android/app-release.apk" 2>/dev/null || echo "0")
    if [ "$APK_SIZE" -lt 1000000 ]; then  # Less than 1MB
        log "❌ CRITICAL: APK size too small ($APK_SIZE bytes) - likely corrupted!"
        exit 1
    fi
    log "✅ APK size validation passed: $APK_SIZE bytes"
else
    log "❌ CRITICAL: APK not found in output directory!"
    exit 1
fi

# Validate APK structure using aapt (if available)
if command -v aapt >/dev/null 2>&1; then
    log "🔍 Validating APK structure..."
    if aapt dump badging "output/android/app-release.apk" >/dev/null 2>&1; then
        log "✅ APK structure validation passed"
    else
        log "❌ CRITICAL: APK structure validation failed - APK is corrupted!"
        exit 1
    fi
else
    log "⚠️ aapt not available, skipping APK structure validation"
fi

# Check for any build artifacts that might indicate corruption
log "🔍 Checking for build corruption indicators..."
if [ -d "build" ] && [ -f "build/app/outputs/flutter-apk/app-release.apk" ]; then
    log "✅ Flutter build artifacts found and valid"
else
    log "⚠️ Flutter build artifacts not found in expected location"
fi
