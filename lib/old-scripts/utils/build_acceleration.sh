...
init_build_acceleration() {
    ...
    export GRADLE_OPTS="-Xmx8G -XX:MaxMetaspaceSize=2G -XX:ReservedCodeCacheSize=1024M -XX:+UseG1GC -XX:MaxGCPauseMillis=50 -XX:+UseStringDeduplication -XX:+OptimizeStringConcat -XX:+TieredCompilation -XX:TieredStopAtLevel=1"
    export JAVA_OPTS="-Xmx8G -XX:MaxMetaspaceSize=2G -XX:+UseG1GC"
    export ANDROID_GRADLE_OPTS="-Xmx8G -XX:MaxMetaspaceSize=2G -XX:+UseG1GC"
    
    # Set Gradle daemon memory
    export GRADLE_DAEMON_OPTS="-Xmx8G -XX:MaxMetaspaceSize=2G -XX:+UseG1GC"
    
    # Enable Gradle daemon and parallel processing
    export GRADLE_DAEMON=true
    export GRADLE_PARALLEL=true
    export GRADLE_BUILD_CACHE=true
    export GRADLE_CONFIGURATION_CACHE=true
...
optimize_gradle_build() {
    ...
    if [ ! -f gradlew ]; then
        ...
        if command -v gradle >/dev/null 2>&1; then
            gradle wrapper --gradle-version=8.12 2>/dev/null || true
        fi
