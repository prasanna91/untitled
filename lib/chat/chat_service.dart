...
class ChatService {
  ...
  Future<void> _fetchAndParsePage(String url) async {
    try {
      ...
      debugPrint('🔗 Storing in knowledge base:');
      debugPrint('   URL: $url');
      debugPrint('   Title: ${content['title'] as String}');
      debugPrint('   Domain: $_currentDomain');
  ...
  Future<ChatResponse> _getAboutResponse() async {
    ...
    for (final entry in _knowledgeBase.values) {
      ...
      if (url.contains('about') && !entry.isStale) {
        aboutContent = _extractRelevantSnippet(
            entry.paragraphs.take(2).join('. '), 'about');
        aboutUrl = entry.url;
    ...
    if (aboutContent.isEmpty && _currentPageEntry != null) {
      aboutContent = _extractRelevantSnippet(
          _currentPageEntry!.paragraphs.take(2).join('. '), 'about');
...
[${contactUrl}](${contactUrl})''',
  ...
  Future<ChatResponse> _getSearchResponse(String query) async {
    ...
    final queryWords =
        query.toLowerCase().split(' ').where((w) => w.length > 2);
    ...
    for (final entry in relevantContent.entries) {
      ...
      final matchedKeywords = allTerms
          .where((term) =>
              content.text.toLowerCase().contains(term.toLowerCase()) ||
              title.toLowerCase().contains(term.toLowerCase()))
          .toList();

      final finalUrl = _ensureValidUrl(entry.key);
      debugPrint('🔗 Creating search result:');
      debugPrint('   Title: $title');
      debugPrint('   URL: $finalUrl');
      debugPrint('   Original key: ${entry.key}');

      searchResults.add(SearchResult(
        title: title,
        content: content.text,
        url: finalUrl,
        favicon: 'https://${Uri.parse(finalUrl).host}/favicon.ico',
        metadata: {
          'keyword': matchedKeywords.isNotEmpty ? matchedKeywords.first : '',
          'matchedTag': content.type,
          'pageTitle': title,
          'domain': Uri.parse(finalUrl).host,
          'timestamp': DateTime.now().toIso8601String(),
        },
        relevance: content.relevance / 100.0,
      ));

      // Format: "results no, Title, next line just two lines of result content text"
      messageLines.addAll([
        'results $index, $title',
        content.text,
        '',
  ...
  Future<Map<String, ContentSection>> _findRelevantContent(String query) async {
    ...
    debugPrint('🔍 Searching knowledge base:');
    debugPrint('   Current domain: $_currentDomain');
    debugPrint('   Knowledge base entries: ${_knowledgeBase.length}');

    for (final entry in _knowledgeBase.values) {
      debugPrint('   Checking entry: ${entry.url} (domain: ${entry.domain})');
      if (entry.domain != _currentDomain || entry.isStale) {
        debugPrint('   Skipping: domain mismatch or stale');
        continue;
      }
    ...
    final sortedResults = Map.fromEntries(results.entries.toList()
  ...
  String _ensureValidUrl(String url) {
    ...
    try {
      ...
      if (uri.scheme.isEmpty) {
        final result = 'https://$url';
        debugPrint('🔗 Added https scheme: $result');
        return result;
      }
